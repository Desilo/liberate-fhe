...
def find_the_next_prime(start: int, m: int, up=True) -> int:
    ...
    while True:
        ok = check_ntt_primality(q=current_query, M=m)
        if ok:
            break
        current_query += step
...
def generate_alternating_prime_sequence(
    ...
    if alternate_directions:
        # Initial search.
        up0 = find_the_next_prime(start=up, m=m)
        down0 = find_the_next_prime(start=down, m=m, up=False)
        ...
        current_direction = False if eup < edown else True
        ...
        while True:
            ...
            current_query = up if current_direction else down
            next_prime = find_the_next_prime(
            ...
            if current_direction:
                ...
                if optimize:
                    ...
                    down = searched if searched < down else down
            else:
                down = next_prime - 2
                if optimize:
                    searched = int((cumulative_scale * scale) // 2 * 2 + 1)
                    up = searched if searched > up else up
        ...
        q_count = 0
        current_query = up if fixed_direction else down
        step = 2 if fixed_direction else -2
        while True:
            current_query = find_the_next_prime(
...
def plot_cumulative_relative_error(
    ...
    how_many = maximum_levels(N=N, qbits=sb)
    p = generate_alternating_prime_sequence(
...
def pgen_pseq(sb, N, how_many: int) -> list | str:
    ...
    try:
        res = generate_alternating_prime_sequence(
            sb=sb, N=N, how_many=how_many
        )
    except Exception as e:
        # Try with the half how_many.
        res = pgen_pseq(sb=sb, N=N, how_many=how_many // 2)
    return res
